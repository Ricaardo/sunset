...
func scheduleSunsetPush() {
	for {
		...
		log.Printf("下次推送时间: %s (还有 %s)", nextRun.Format("2006-01-02 15:04:05"), duration)
...
func getNextPushTime() time.Time {
	...
	if config.UseSunsetTime {
		...
		// 在日落前提前推送
		nextRun = sunsetTime.Add(-time.Duration(config.SunsetAdvanceMinutes) * time.Minute)
		...
		nextRun = time.Date(now.Year(), now.Month(), now.Day(),
			config.ScheduleHour, config.ScheduleMinute, 0, 0, beijingLocation)
...
func main() {
	...
	http.HandleFunc("/sunset-time", sunsetTimeHandler)
